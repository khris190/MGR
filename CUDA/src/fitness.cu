...
float calculateFitness(unsigned char *img_data, unsigned char *surface_data, int _width, int _height){
    ...
    //increasing of this number lowers calculate fitness cpu timings 2 times and increases calculateFitnessFromArray 4 times, so its important to think about those numbers
