...
{
    {
        ...
        if (!std::filesystem::exists(Config::get<Config::Argument::INPUT>())) {
            logger.logErr(
                "no input file: " + std::filesystem::absolute(Config::get<Config::Argument::INPUT>()).string());
            return 2;
        }
        logger.logInfo("Starting");
        ArtGeneration gen(Config::get<Config::Argument::POPULATION>(),
            Config::get<Config::Argument::SHAPE_AMOUNT>());
        logger.logInfo("cairo_image_surface_create_from_png");
        cairo_surface_t* image = cairo_image_surface_create_from_png(
            Config::get<Config::Argument::INPUT>().c_str());
        ...
        for (int y = 0; y < height; y++) {
            memcpy(flippedPixels.data() + (y * width * 4),
                imgData + ((height - y - 1) * width * 4), width * 4);
        }
        int stride = cairo_image_surface_get_stride(image);
        cairo_surface_destroy(image);
        image = cairo_image_surface_create_for_data(
            flippedPixels.data(), CAIRO_FORMAT_ARGB32, width, height, stride);
