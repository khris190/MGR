...
void ArtGeneration::draw(cairo_surface_t *img, size_t index) {
  this->population.children[index].draw(img,
...
void ArtGeneration::startEvolution(cairo_surface_t *img) {
  ...
  do {
    ...
    if (0 > this->population.bests[1].first ||
        0 > this->population.bests[0].first) {
      logger.logEmerg("wtf, parent1_ or parent2_ wasnt selected");
      exit(2);
    }
    this->population.createNextGeneration(
        Config::get<Config::Argument::MUTATION>());
    bestScore = this->population.bests[1].second;

    logger.logInfo(("Score: " + std::to_string(bestScore) + "\nDifference: " +
                    std::to_string(bestScore - savedBestScore))
                       .c_str());

    if (bestScore >= savedBestScore) {
      savedBestScore = bestScore;
      if (mutationsCounter % 2500 == 2000) {
        openGLDrawer::drawSecond(
            this->population.children[this->population.bests[1].first]);
        openGLDrawer::saveToPNG(
            Config::getOutputFilePathAndFileName(savedBestScore).c_str());
        logger.logDeb(Profiler::getInstance()->getTimingsAsString().c_str());
      }
      mutationsCounter++;
    }
  } while (bestScore < Config::get<Config::Argument::RESEMBLENCE>() &&
           !Config::doStop() &&
           !glfwWindowShouldClose(
               openGLDrawer::OGlhandler->mainWindow->getWindow()));
