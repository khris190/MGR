...
void Population::drawNFitness(cairo_surface_t *img) {
  ...
  openGLDrawer::draw(this->children[0], scale);
  for (size_t i = 0; i < this->children.size() - 1; i++) {
    openGLDrawer::draw(this->children[i + 1], scale);

    images[i] = openGLDrawer::getPixels();
    workers[i] = pool.submit(myFitness, img, images[i].data());
  }

  openGLDrawer::draw(this->children[size - 1], scale);

  images[size - 1] = openGLDrawer::getPixels();
  workers[size - 1] = pool.submit(myFitness, img, images[size - 1].data());
  for (size_t i = 0; i < this->children.size(); i++) {
