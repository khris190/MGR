...
namespace Config
{
    ...
    bool doStop(int SecondsAfterToStop) { return (time(nullptr) - start_time) > (int)(3600 * get<Argument::HOURS>()); }

    void CreateFolderForOutput()
    {
        std::string dir = "./in:" + get<Argument::INPUT>() + "." + std::to_string(time(nullptr)) + "/";
        struct stat st = { 0 };
        ...
        MyFile << "input_name: " << get<Argument::INPUT>() << std::endl
               << "output_name: " << get<Argument::OUTPUT>() << std::endl
               ...
               << "shape_types: " << std::bitset<8>(get<Argument::SHAPE_TYPES>()) << std::endl
    ...
    {
        return out_folder_name + "/" + std::to_string(Resemblance) + get<Argument::INPUT>();
    }
} // namespace Config
